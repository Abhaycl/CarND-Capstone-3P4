...
def _extractBox(boxes, scores, classes, confidence, img_width, img_height):
    ...
    if number != -1:
        ...
        left, right, top, bottom = (min_x * img_width, max_x * img_width, min_y * img_height, max_y * img_height)
...
class TLClassifier(object):
    ...
    def detection(self, img):
        ...
        with self.sess_detection.as_default(), self.graph_detection.as_default(), Timer('detection'):
            boxes, scores, classes = self.sess_detection.run([self.detection_boxes, self.detection_scores, self.detection_classes],
                                                              feed_dict = {self.image_tensor: img_expanded})
            
            # Extract box and append to list.
            return _extractBox(boxes, scores, classes, 0.1, img_width, img_height)
    ...
    def classification(self, img):
        ...
        with self.sess_classification.as_default(), self.graph_classification.as_default(), Timer('classification'):
            sfmax = list(self.sess_classification.run(tf.nn.softmax(self.out_graph.eval(feed_dict = {self.in_graph: [img]}))))
            sfmax_ind = sfmax.index(max(sfmax))
            
            # Add a colored bbox and publish traffic light if needed - rosparam set /tl_detector/publish_traffic_light true.
            if rospy.get_param('~publish_traffic_light', False):
                cv2.rectangle(img, (0, 0), (31, 31), self.index2color[sfmax_ind], 1)
                self.traffic_light_pub.publish(self.bridge.cv2_to_imgmsg(img, "rgb8"))
        
        return self.index2msg[sfmax_ind]
